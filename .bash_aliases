...
function g {
  if [[ $# > 0 ]]; then
    ...
    echo
    [[ -n $(git branch-status) ]] && (git branch-status; echo)
    echo -e "   $(tput setaf 0)Last commit:     $(tput sgr0)$(time_since_last_commit)"
    echo -e "   $(tput setaf 0)Most recent ref: $(tput sgr0)$(git describe --all 2>/dev/null)"
    echo
